...
function Application(options) {
  ...
  this.createFileOnLaunch = options.createFileOnLaunch;
    ...
    const stopApp = async () => {
      try {
        if (this.api.nodeIntegration) {
          await this.client.electron.remote.app.quit();
        } else {
          await this.client.windowByIndex(0);
...
Application.prototype.createClient = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (self.createFileOnLaunch) {
      args.push('spectron-create-file=' + self.createFileOnLaunch);
    }
