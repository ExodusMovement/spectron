...
Api.prototype.isNodeIntegrationEnabled = async function () {
  const self = this;
  const windowCount = await self.app.client.getWindowCount();
  for (let windowIdx = 0; windowIdx <= windowCount; windowIdx++) {
    await self.app.client.windowByIndex(windowIdx);
    const result = await self.app.client.execute(function (requireName) {
